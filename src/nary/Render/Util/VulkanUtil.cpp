...
//  nary
//
//  Created by Ninter6 on 2024/2/18.
//
...
void VulkanUtil::createBufferAndInitialize(VkDevice              device,
    ...
    if (VK_SUCCESS != vkCreateBuffer(device, &bufferCreateInfo, nullptr, buffer)) {
        ERROR_LOG("create buffer buffer failed!");
        return;
    }
    ...
    LOOP(deviceMemoryProperties.memoryTypeCount) {
    ...
    if (!memTypeFound) {
        ERROR_LOG("memTypeFound is nullptr");
        return;
    }
    if (VK_SUCCESS != vkAllocateMemory(device, &memAlloc, nullptr, memory)) {
        ERROR_LOG("alloc memory failed!");
        return;
    }
...
void VulkanUtil::createBuffer(VkPhysicalDevice      physical_device,
    ...
    if (vkCreateBuffer(device, &buffer_create_info, nullptr, &buffer) != VK_SUCCESS) {
        ERROR_LOG("vkCreateBuffer failed!");
        return;
    }
    ...
    if (vkAllocateMemory(device, &buffer_memory_allocate_info, nullptr, &buffer_memory) != VK_SUCCESS) {
        ERROR_LOG("vkAllocateMemory failed!");
        return;
    }
...
void VulkanUtil::copyBuffer(naDevice&    device,
                            VkBuffer     srcBuffer,
                            VkBuffer     dstBuffer,
...
void VulkanUtil::createGlobalImage(naDevice&          device,
                                   VkImage&           image,
                                   VkImageView&       image_view,
    ...
    switch (texture_image_format) {
        ...
        case VK_FORMAT_R32G32B32A32_SFLOAT:
            ...
            break;
        default:
            ERROR_LOG("invalid texture_byte_size");
            break;
    }
    ...
    VulkanUtil::createBuffer(device.physicalDevice(),
                             device.device(),
                             texture_byte_size,
    ...
    void* data;
    vkMapMemory(
                device.device(), inefficient_staging_buffer_memory, 0, texture_byte_size, 0, &data);
    memcpy(data, texture_image_pixels, static_cast<size_t>(texture_byte_size));
    vkUnmapMemory(device.device(), inefficient_staging_buffer_memory);
    ...
    vmaCreateImage(device.assetAllocator(),
                   &image_create_info,
                   &allocInfo,
                   &image,
                   &image_allocation,
    ...
    copyBufferToImage(device, inefficient_staging_buffer, image, texture_image_width, texture_image_height, 1);
    ...
    vkDestroyBuffer(device.device(), inefficient_staging_buffer, nullptr);
    vkFreeMemory(device.device(), inefficient_staging_buffer_memory, nullptr);
    
    // generate mipmapped image
    genMipmappedImage(device, image, texture_image_width, texture_image_height, mip_levels);
    
    image_view = createImageView(device.device(),
                                 image,
                                 vulkan_image_format,
                                 VK_IMAGE_ASPECT_COLOR_BIT,
...
void VulkanUtil::createCubeMap(naDevice&            device,
                               VkImage&             image,
                               VkImageView&         image_view,
                               VmaAllocation&       image_allocation,
                               uint32_t             texture_image_width,
    ...
    switch (texture_image_format) {
        ...
        default:
            ...
            ERROR_LOG("invalid texture_layer_byte_size");
            return;
            break;
    }
    ...
    void* data = NULL;
    vkMapMemory(
                device.device(), inefficient_staging_buffer_memory, 0, cube_byte_size, 0, &data);
    LOOP(6) {
    ...
    generateTextureMipMaps(
                           device, image, vulkan_image_format, texture_image_width, texture_image_height, 6, miplevels);
...
void VulkanUtil::generateTextureMipMaps(naDevice&   device,
                                        VkImage     image,
    ...
    vkGetPhysicalDeviceFormatProperties(device.physicalDevice(), image_format, &format_properties);
    if (!(format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
        ERROR_LOG("generateTextureMipMaps() : linear bliting not supported!");
        return;
    }
    VkCommandBuffer command_buffer = device.beginSingleTimeCommands();
    ...
    LOOPB(miplevels, 1) // use miplevel i-1 to generate miplevel i and set i-1 to
        // shader_read
    {
...
void VulkanUtil::transitionImageLayout(naDevice&          device,
                                       VkImage            image,
    ...
    // for generating mipmapped image
        ...
        ERROR_LOG("unsupported layout transition!");
        return;
    }
...
void VulkanUtil::copyBufferToImage(naDevice&    device,
                                   VkBuffer     buffer,
                                   VkImage      image,
                                   uint32_t     width,
                                   uint32_t     height,
...
void VulkanUtil::genMipmappedImage(naDevice& device, VkImage image, uint32_t width, uint32_t height, uint32_t mip_levels) {
    VkCommandBuffer command_buffer = device.beginSingleTimeCommands();
...
VkSampler VulkanUtil::getOrCreateMipmapSampler(VkPhysicalDevice physical_device,
    ...
    if (find_sampler != m_mipmap_sampler_map.end()) {
        ...
        {
            ERROR_LOG("vkCreateSampler failed!");
        }
    }
...
void VulkanUtil::clear(VkDevice device) {
    destroyNearestSampler(device);
    destroyLinearSampler(device);
    destroyMipmappedSampler(device);
}
